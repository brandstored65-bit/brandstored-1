...
function WishlistAuthed() {
    ...
    const removeFromWishlist = async (productId) => {
        try {
            if (!isSignedIn) {
                ...
                const guestWishlist = JSON.parse(localStorage.getItem('guestWishlist') || '[]');
                const updated = guestWishlist.filter(item => item.productId !== productId);
    ...
    return (
        <>
            ...
            <div className={`max-w-7xl mx-auto px-4 py-8 ${isSignedIn ? 'grid grid-cols-1 md:grid-cols-4 gap-6' : ''}`}>
